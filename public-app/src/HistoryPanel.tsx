...
export function HistoryPanel({
  ...
  onSelect?: (msgs: HistoryMessage[]) => void;
}) {
          ...
          <div style={{ padding: 4 }}>
            ...
            {conversations.map((conv) => (
              <div
                key={conv.id}
                onClick={() => {
                  // Find this conversation's messages (from this user message to next user message)
                  const idx = messages.findIndex((m) => m.id === conv.id);
                  if (idx >= 0 && onSelect) {
                    // Find the next user message after this one
                    let nextIdx = messages.findIndex(
                      (m, i) => i > idx && m.role === "user",
                    );
                    if (nextIdx === -1) nextIdx = messages.length;
                    // Extract just this conversation
                    const convMsgs = messages.slice(idx, nextIdx);
                    onSelect(convMsgs);
                  }
                  setOpen(false);
                }}
