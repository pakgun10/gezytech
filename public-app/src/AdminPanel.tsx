...
function useAdminApi(token: string) {
  ...
  return { approveToolRequest, rejectToolRequest, approveSoulRequest, rejectSoulRequest }
}
...
export function AdminPanel() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      fetch(`${API_BASE}/users`, { headers }).then((r) => r.ok ? r.json() : null),
      fetch(`${API_BASE}/tool-requests`, { headers }).then((r) => r.ok ? r.json() : null),
      fetch(`${API_BASE}/soul-requests`, { headers }).then((r) => r.ok ? r.json() : null),
    ])
      .then(([u, t, s]) => {
  ...
  const statusBadge = (s: string) => {
    const colors: Record<string, string> = {
      pending: '#f59e0b', approved: '#16a34a', rejected: '#dc2626',
    }
    return (
      <span style={{
  ...
  if (!authOk) {
    return (
      <div style={{ maxWidth: 400, margin: '60px auto', padding: 20 }}>
        ...
        {error && (
          <div style={{ padding: '8px 12px', background: '#fef2f2', color: '#dc2626', borderRadius: 6, marginBottom: 12, fontSize: 13 }}>
            {error}
          </div>
        )}
        <input
          ...
          onKeyDown={(e) => e.key === 'Enter' && token && fetch(`${API_BASE}/users`, { headers: { 'x-admin-token': token } }).then((r) => r.ok ? setAuthOk(true) : setError('Invalid token'))}
        />
        <button
          onClick={() => {
            if (!token) return
  ...
  const pendingSouls = soulRequests.filter((r) => r.status === 'pending').length
  ...
  return (
    <div style={{ maxWidth: 900, margin: '0 auto', padding: 20, fontFamily: 'system-ui' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
        ...
        <button
          onClick={() => { setAuthOk(false); setToken(''); localStorage.removeItem('admin_token') }}
          style={{ padding: '4px 12px', background: 'transparent', color: '#ef4444', border: '1px solid #ef4444', borderRadius: 6, cursor: 'pointer', fontSize: 12 }}
        >
          Lock
        </button>
      </div>
      ...
      {/* Tabs */}
      <div style={{ display: 'flex', gap: 4, marginBottom: 20, borderBottom: '2px solid #e5e5e5' }}>
        {([
        ...
        <div>
          {toolRequests.length === 0 ? (
            <div className="empty-state">
              ...
              <div className="empty-state-desc">Users haven't requested any tools yet.</div>
            </div>
          ) : (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              {toolRequests.map((req) => (
                <div key={req.id} style={{
                  padding: 14, background: '#fff', border: '1px solid #e5e5e5',
                  borderRadius: 8, display: 'flex', justifyContent: 'space-between',
                  alignItems: 'flex-start', gap: 12,
                }}>
                  <div style={{ flex: 1 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
                      <span style={{ fontWeight: 600, fontSize: 15 }}>{req.toolName}</span>
                    ...
                    <div style={{ fontSize: 11, color: '#9ca3af' }}>
                      User: {req.userId.slice(0, 8)}... · {formatDate(req.createdAt)}
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 6, minWidth: 160 }}>
                      ...
                      <div style={{ display: 'flex', gap: 4 }}>
                        <button
                          ...
                          style={{
                            flex: 1, padding: '4px 10px', background: '#16a34a', color: '#fff',
                        ...
                        <button
                          ...
                          style={{
                            flex: 1, padding: '4px 10px', background: '#dc2626', color: '#fff',
                            border: 'none', borderRadius: 6, fontSize: 12, fontWeight: 600,
                            cursor: 'pointer',
                          }}
                        >
        ...
        <div>
          {soulRequests.length === 0 ? (
            <div className="empty-state">
              ...
              <div className="empty-state-desc">Users haven't requested persona changes yet.</div>
            </div>
          ) : (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              {soulRequests.map((req) => (
                <div key={req.id} style={{
                  ...
                  <div style={{ flex: 1 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
                      <span style={{ fontWeight: 600, fontSize: 15 }}>SOUL Request</span>
                      {statusBadge(req.status)}
                    </div>
                    <div style={{
        ...
        <div>
          {users.length === 0 ? (
            <div className="empty-state">
              ...
              <div className="empty-state-desc">Create users via the admin API.</div>
            </div>
          ) : (
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
              <thead>
                <tr style={{ background: '#f9fafb', textAlign: 'left' }}>
                  <th style={{ padding: '8px 12px', borderBottom: '1px solid #e5e5e5' }}>Email</th>
                  <th style={{ padding: '8px 12px', borderBottom: '1px solid #e5e5e5' }}>Name</th>
                  <th style={{ padding: '8px 12px', borderBottom: '1px solid #e5e5e5' }}>Agent</th>
                  <th style={{ padding: '8px 12px', borderBottom: '1px solid #e5e5e5' }}>Created</th>
                </tr>
              </thead>
              <tbody>
                {users.map((u) => (
                  <tr key={u.id}>
                    <td style={{ padding: '8px 12px', borderBottom: '1px solid #f3f4f6' }}>{u.email}</td>
                    <td style={{ padding: '8px 12px', borderBottom: '1px solid #f3f4f6' }}>{u.displayName ?? '-'}</td>
                    <td style={{ padding: '8px 12px', borderBottom: '1px solid #f3f4f6' }}>
                      <code style={{ background: '#f3f4f6', padding: '1px 6px', borderRadius: 4, fontSize: 12 }}>{u.agentSlug}</code>
                    </td>
                    <td style={{ padding: '8px 12px', borderBottom: '1px solid #f3f4f6', color: '#9ca3af' }}>
