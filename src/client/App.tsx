...
const ChatPage = lazy(() =>
  import("@/client/pages/chat/ChatPage").then((m) => ({ default: m.ChatPage })),
);
const ProjectsPage = lazy(() =>
  import("@/client/pages/projects/ProjectsPage").then((m) => ({
    default: m.ProjectsPage,
  })),
);
const BooksPage = lazy(() =>
  import("@/client/pages/books/BooksPage").then((m) => ({
    default: m.BooksPage,
  })),
);
const BookReader = lazy(() =>
  import("@/client/pages/books/BookReader").then((m) => ({
    default: m.BookReader,
  })),
);
const TasksPage = lazy(() =>
  import("@/client/pages/tasks/TasksPage").then((m) => ({
    default: m.TasksPage,
  })),
);
const CronsPage = lazy(() =>
  import("@/client/pages/crons/CronsPage").then((m) => ({
    default: m.CronsPage,
  })),
);
const FilesPage = lazy(() =>
  import("@/client/pages/files/FilesPage").then((m) => ({
    default: m.FilesPage,
  })),
);
const MiniAppsPage = lazy(() =>
  import("@/client/pages/mini-apps/MiniAppsPage").then((m) => ({
    default: m.MiniAppsPage,
  })),
);
const ModelRegistryPage = lazy(() =>
  import("@/client/pages/models/ModelRegistryPage").then((m) => ({
    default: m.ModelRegistryPage,
  })),
);
const TerminalPage = lazy(() =>
  import("@/client/pages/terminal/TerminalPage").then((m) => ({
    default: m.TerminalPage,
  })),
);
const LoginPage = lazy(() =>
  import("@/client/pages/login/LoginPage").then((m) => ({
    default: m.LoginPage,
  })),
);
const OnboardingPage = lazy(() =>
  import("@/client/pages/onboarding/OnboardingPage").then((m) => ({
    default: m.OnboardingPage,
  })),
);
const DesignSystemPage = lazy(() =>
  import("@/client/pages/design-system/DesignSystemPage").then((m) => ({
    default: m.DesignSystemPage,
  })),
);
const InvitePage = lazy(() =>
  import("@/client/pages/invite/InvitePage").then((m) => ({
...
const SettingsModal = lazy(() =>
  import("@/client/pages/settings/SettingsPage").then((m) => ({
    default: m.SettingsModal,
  })),
);
const AccountDialog = lazy(() =>
  import("@/client/pages/account/AccountPage").then((m) => ({
  ...
  return (
    <div className="surface-base flex min-h-screen items-center justify-center">
      <div className="text-center animate-fade-in">
        <h1 className="gradient-primary-text text-4xl font-bold tracking-tight">
          Gezy
        </h1>
      </div>
    </div>
...
function AuthenticatedShell() {
  ...
  const [settingsInitialSection, setSettingsInitialSection] = useState<
    string | undefined
  >();
  const [settingsFilters, setSettingsFilters] = useState<
    { agentId?: string } | undefined
  >();
  const [accountOpen, setAccountOpen] = useState(false);

  const handleOpenSettings = useCallback(
    (section?: string, filters?: { agentId?: string }) => {
  ...
  useEffect(() => {
    ...
    if (connected) {
      toast.success(
        t("settings.emailAccounts.connectedToast", { email: connected }),
      );
                      ...
                      <div className="min-w-0 flex-1">
                        <Suspense fallback={<PageFallback />}>
                          <Routes>
                            ...
                            />
                            <Route
                              path="/files/:sourceType/:sourceId"
                              element={<FilesPage />}
                            />
                            <Route
                              path="/mini-apps"
                              element={<MiniAppsPage />}
                            />
                            <Route
                              path="/models"
