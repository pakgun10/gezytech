...
function getQueenieKnowledge(): string {
  if (cachedQueenieKnowledge === null) {
    try {
      cachedQueenieKnowledge = readFileSync(
        join(import.meta.dir, "..", "assets", "queenie-knowledge.md"),
        "utf-8",
      ).trim();
    } catch {
...
function buildContextBlock(): string {
  ...
  const installLine = env
    ? (installLabel[env.installationType] ?? env.installationType)
    : "unknown";
 ...
 */
function buildCurrentMessageHint(
  source: PromptParams["currentMessageSource"],
): string | null {
  ...
  const formatHints: Record<string, string> = {
    discord:
      "Supports Markdown. No tables — use lists. Wrap URLs in <> to suppress embeds.",
 ...
 */
function buildConversationStateBlock(
  state: PromptParams["conversationState"],
): string | null {
  ...
  if (state.hasCompactedHistory) {
    ...
    lines.push(
      `You can see the ${state.visibleMessageCount} most recent message${state.visibleMessageCount !== 1 ? "s" : ""} in full detail.`,
    );
  } else {
    lines.push(
      `You have the full conversation history: ${state.visibleMessageCount} message${state.visibleMessageCount !== 1 ? "s" : ""}.`,
    );
  }
 ...
 */
function buildSubjectGroupedMemories(
  header: string,
  memories: Memory[],
): string {
 ...
 */
function buildCategoryGroupedMemories(
  header: string,
  memories: Memory[],
): string {
  const groups = new Map<string, Memory[]>();
...
function renderProjectKnowledgeBlock(
  ...
  if (pinnedItems.length === 0 && indexItems.length === 0 && totalCount === 0)
    return null;
  ...
  if (pinnedItems.length > 0) {
    const pinnedLines: string[] = [
      `### Pinned knowledge (${pinnedItems.length})`,
      "",
    ];
    pinnedLines.push(
...
function buildActiveProjectBlock(info: ActiveProjectPromptInfo): string {
  ...
  if (info.tags.length > 0) {
    const tagLines = info.tags
      .map((t) => `- ${t.label} (${t.color})`)
      .join("\n");
    sections.push(`### Tags\n\n${tagLines}`);
  }

  const knowledgeBlock = renderProjectKnowledgeBlock(
    info.pinnedKnowledge,
    info.knowledgeIndex,
    info.totalKnowledgeCount,
  );
  ...
  if (info.openTickets.length > 0) {
    const ticketLines = info.openTickets
      .map((t) => {
        const tagPart =
          t.tagLabels.length > 0 ? ` — ${t.tagLabels.join(", ")}` : "";
...
function buildSystemContextBlock(
  ...
  if (workspacePath) {
    lines.push(
      `- Workspace: ${workspacePath} (default cwd for run_shell — pass it via the \`cwd\` parameter, not via \`cd ... &&\` prefixes)`,
    );
  }
...
function buildTicketAssignmentBlock(info: TicketAssignmentInfo): string {
  ...
  if (info.projectGithubUrl)
    projectHeader += `\nGitHub: ${info.projectGithubUrl}`;
  const projectDesc =
    info.projectDescription.trim().length > 0
  ...
  const tagsLine =
    info.ticketTags.length > 0 ? `\nTags: ${info.ticketTags.join(", ")}` : "";
  const descriptionLine =
    info.ticketDescription.trim().length > 0
  ...
  if (info.taskHistory && info.taskHistory.length > 0) {
    ...
    for (const task of info.taskHistory) {
      ...
      if (task.result?.trim()) {
        ...
        lines.push(
          `  No result or error summary is stored. Use get_task_detail(task_id: "${task.id}") or get_task_messages(task_id: "${task.id}", offset: -20) if you need to inspect where this task stopped.`,
        );
      }
  ...
  if (info.comments && info.comments.length > 0) {
    const blocks: string[] = [
      "### Existing comments on this ticket (chronological)",
    ];
...
export function buildSystemPrompt(params: PromptParams): BuiltSystemPrompt {
  ...
  if (params.isSubAgent && params.taskDescription) {
    ...
    if (params.systemContext) {
      stableBlocks.push(
        buildSystemContextBlock(params.systemContext, params.workspacePath),
      );
    }
    ...
    if (!onTicketTask) {
      ...
      constraintsLines.push(
        `- Communicate via the ticket. Use update_ticket() to update status/description/tags; use prompt_human() to ask the user a question (the task is suspended with a yellow "awaiting input" badge on the ticket until they answer). For structured choices use prompt_human's confirm/select/multi_select; for free-form answers use prompt_type="text" — or call request_input() which routes through the same human-prompt flow on ticket tasks.`,
      );
      constraintsLines.push(
        `- Do NOT report intermediate progress to a parent Agent — there is none on ticket tasks. Your audience is the user reading the ticket.`,
      );
      constraintsLines.push(
    ...
    stableBlocks.push(
      `You are ${params.agent.name}${slugSuffix}, ${params.agent.role}.`,
    );
    ...
    stableBlocks.push(
      `## Core principles\n\n` +
        ...
        `- Be resourceful before asking — check your memory, contacts, and available tools before requesting clarification. **Exception:** when user instructions conflict with each other, do NOT choose between them yourself — ask the user to clarify.\n` +
        `- Obey user instructions faithfully. Negative instructions ("do not", "stop", "never") take precedence over positive instructions ("do", "explain", "write"). If obeying a negative instruction means you cannot fulfill a later instruction, the negative instruction wins.\n` +
        `- Unless the user explicitly asks for explanation or elaboration, respond concisely and directly. Do not add commentary or justification after completing the requested action.\n` +
    ...
    if (params.agent.character) {
      stableBlocks.push(
        `## Personality\n\n` +
          ...
          `---\n` +
          `⚠️ The personality rules above take precedence over any conflicting guidance in other sections of this system prompt, including Core Principles. When this Personality section and another section disagree, follow this Personality.`,
      );
    } else {
      stableBlocks.push(
        `## Personality\n\n` +
  ...
  if (params.isQuickSession) {
    ...
    volatileBlocks.push(buildContextBlock());
  ...
  if (toolsEnabled && params.mcpTools && params.mcpTools.length > 0) {
    const mcpLines = params.mcpTools
      .map(
        (server) => `- **${server.serverName}** (${server.tools.length} tools)`,
      )
      .join("\n");
    stableBlocks.push(
  ...
  if (
    !params.isSubAgent &&
    params.accountTriggers &&
    params.accountTriggers.length > 0
  ) {
    const triggerLines = params.accountTriggers
      .map(
        (t) =>
          `- **${t.name}** on ${t.accountLabel} (${t.folder}) — when ${t.conditionsSummary} → ${t.dispatchMode === "task" ? "a task is spawned" : "injected into this conversation"}`,
      )
      .join("\n");
    stableBlocks.push(
  ...
  if (params.compactingSummaries && params.compactingSummaries.length > 0) {
    const summaryBlocks = params.compactingSummaries.map((s) => {
      const fromDate = s.firstMessageAt.toLocaleDateString("en-US", {
        month: "short",
        day: "numeric",
        timeZone: config.timezone,
      });
      const toDate = s.lastMessageAt.toLocaleDateString("en-US", {
        month: "short",
        day: "numeric",
        year: "numeric",
        timeZone: config.timezone,
