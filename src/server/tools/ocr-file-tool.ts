        ...
        try {
          ...
          if (!trimmed) {
            return {
              ...
              text: "",
              warning:
                "OCR completed but no text was found in the image. The image may not contain readable text (e.g., it could be a photo without text).",
            };
          }
          ...
          if (msg.includes("tesseract") && msg.includes("not found")) {
            return {
              success: false,
              error:
                "tesseract is not installed on the server. Install with: sudo apt install tesseract-ocr",
            };
          }
          return {
            success: false,
