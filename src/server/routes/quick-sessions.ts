...
agentScopedRoutes.post("/", async (c) => {
  ...
  if (activeCount.length >= config.quickSessions.maxActivePerUserPerAgent) {
    return c.json(
      {
        error: {
          code: "MAX_QUICK_SESSIONS",
          message: "Maximum active quick sessions reached for this Agent",
        },
      },
      409,
    );
  ...
  const session = {
    ...
    expiresAt: new Date(
      now.getTime() +
        config.quickSessions.defaultExpirationHours * 60 * 60 * 1000,
    ),
  };
  ...
  log.debug(
    { agentId, sessionId: session.id, userId: user.id },
    ...
    return c.json(
      {
        error: {
          ...
          message: "Quick session not found",
        },
      },
      404,
    );
  ...
  return c.json({
    session: {
      ...
      closedAt: session!.closedAt
        ? (session!.closedAt as Date).getTime()
        : null,
      expiresAt: session!.expiresAt
        ? (session!.expiresAt as Date).getTime()
        : null,
      ...
      thinkingEffort:
        (session!.thinkingEffort as QuickSessionSummary["thinkingEffort"]) ??
        null,
    } satisfies QuickSessionSummary,
    ...
    return c.json(
      {
        error: {
          code: "SESSION_CLOSED",
          message: "This quick session is closed",
        },
      },
      409,
    );
      ...
      return c.json(
        {
          error: {
            ...
            message: "model must be a non-empty string or null",
          },
        },
        400,
      );
      ...
      return c.json(
        {
          error: {
            code: "VALIDATION_ERROR",
            message: "thinkingEnabled must be a boolean or null",
          },
        },
        400,
      );
      ...
      return c.json(
        {
          error: {
            code: "VALIDATION_ERROR",
            message: `thinkingEffort must be one of ${THINKING_EFFORTS.join(" | ")} or null`,
          },
        },
        400,
      );
  ...
  return c.json({
    session: {
      ...
      closedAt: updated!.closedAt
        ? (updated!.closedAt as Date).getTime()
        : null,
      expiresAt: updated!.expiresAt
        ? (updated!.expiresAt as Date).getTime()
        : null,
      model: updated!.model ?? null,
      providerId: updated!.providerId ?? null,
      thinkingEnabled: updated!.thinkingEnabled ?? null,
      thinkingEffort:
        (updated!.thinkingEffort as QuickSessionSummary["thinkingEffort"]) ??
        null,
    } satisfies QuickSessionSummary,
  });
});
...
sessionRoutes.post("/:id/messages", async (c) => {
    ...
    return c.json(
      {
        error: {
          ...
          message: "This quick session has expired",
        },
      },
      409,
    );
    ...
    return c.json(
      {
        error: {
          ...
          message: "Message content must be 100,000 characters or less",
        },
      },
      400,
    );
      ...
      return c.json(
        {
          error: {
            ...
            message: "Each fileId must be a valid UUID",
          },
        },
        400,
      );
  ...
  log.debug(
    { sessionId: session!.id, agentId: session!.agentId, messageId: id },
    "Quick session message enqueued",
  );
...
sessionRoutes.post("/:id/close", async (c) => {
  ...
  const body = await c.req.json().catch(() => ({}));
  const { saveMemory, memorySummary: rawSummary } = body as {
    saveMemory?: boolean;
    ...
    return c.json(
      {
        error: {
          ...
          message: "Memory summary must be 5000 characters or less",
        },
      },
      400,
    );
    ...
    return c.json(
      {
        error: {
          ...
          message: "A summary is required when saving as memory",
        },
      },
      400,
    );
...
export {
  agentScopedRoutes as quickSessionAgentRoutes,
  sessionRoutes as quickSessionDetailRoutes,
};
